...
public class Cipher {
                ...
                if (!MyUtil.isWhiteList(packageName)) {
                    ...
                    if (!monPackageName.equals("")) {
                        if (packageName.equals(monPackageName)) {
                            ...
                            } else if (mode == UNWRAP_MODE) {
                                modeString = "UNWRAP_MODE";
                            } else if (mode == WRAP_MODE) {
    ...
    public final byte[] doFinal() throws IllegalBlockSizeException,
        ...
        //add by icew4y 2019 12 13
        byte[] resultData = getSpi().engineDoFinal(null, 0, 0);
        ...
        if (switch_state == true ) {
            try {
                ...
                if (!MyUtil.isWhiteList(packageName)) {
                    ...
                    if (!monPackageName.equals("")) {
                        if (packageName.equals(monPackageName)) {
                            if (this.specifiedProvider != null) {
                                jsoninfo.put("provider", this.specifiedProvider.getName());
                            }else{
    ...
    public final byte[] doFinal(byte[] input) throws IllegalBlockSizeException,
        ...
        //add by icew4y 2019 12 13
        byte[] resultData = getSpi().engineDoFinal(input, 0, input.length);

        if (switch_state == true && !check_oom(input)) {
            try {
                String packageName = ContextHolder.getPackageName();
                if (!MyUtil.isWhiteList(packageName)) {
                    if (monPackageName.equals("")) {
    ...
    public final byte[] doFinal(byte[] input, int inputOffset, int inputLen)
        ...
        byte[] resultData = getSpi().engineDoFinal(input, inputOffset, inputLen);
        if (switch_state == true  && !check_oom(input)) {
            try {
                String packageName = ContextHolder.getPackageName();
                if (!MyUtil.isWhiteList(packageName)) {
    ...
    public final int doFinal(byte[] input, int inputOffset, int inputLen,
        ...
        int ret = getSpi().engineDoFinal(input, inputOffset, inputLen, output,
                outputOffset);
    ...
    public final int doFinal(ByteBuffer input, ByteBuffer output)
        ...
        int ret = getSpi().engineDoFinal(input, output);
        if (switch_state == true) {
            try {
                String packageName = ContextHolder.getPackageName();
                if (!MyUtil.isWhiteList(packageName)) {
